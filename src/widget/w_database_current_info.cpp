...
{
  if (!Database_Manager::is_valid()) return;

  QLocale      lang     = Database_Manager::current_database()->manifest().language;
  CurrencyInfo currency = Database_Manager::current_database()->manifest().currency;

  ui->le_currency->setText(currency.to_str());
  ui->le_datetime->setText(
      Database_Manager::current_database()->manifest().last_save_time.toString("dd/MM/yyyy HH:mm:ss"));
  ui->le_language->setText(lang.nativeLanguageName());
  ui->le_name->setText(Database_Manager::current_database()->manifest().name);
}
