...
W_Invoice::W_Invoice(qsizetype id)
  : QWidget(nullptr)
  , id(id)
  , ui(new Ui::W_Invoice)
{
...
{
  auto invoice = Invoice::read_record(id);
  if (invoice) (void)invoice.delete_record();
}


void W_Invoice::on_b_edit_clicked()
{
  auto creator = W_Invoice_Creator(id);
...
void W_Invoice::on_b_print_clicked()
{
  ...
  qsizetype property_id = Rent::load_from_tenant(invoice.tenant_id, invoice.start_date).property_id;
  ...
  auto* w = new W_Invoice_Form(id, rent_id_linked);
  w->render(&painter);
  painter.end();
