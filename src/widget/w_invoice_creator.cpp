...
void W_Invoice_Creator::clear()
{
  ...
  ui->dsb_charge_collected->setPrefix(Database_Manager::current_database()->manifest().currency.symbol);
  ui->dsb_other->setValue(0.0F);
  ui->dsb_other->setPrefix(Database_Manager::current_database()->manifest().currency.symbol);
  ui->dsb_water_price->setValue(1.0F);
  ui->dsb_water_price->setPrefix(Database_Manager::current_database()->manifest().currency.symbol);
  ui->dsb_water_subscription->setValue(0.0F);
  ui->dsb_water_subscription->setPrefix(Database_Manager::current_database()->manifest().currency.symbol);
...
{
  if (!RentalisSettings::invoice_warning("invoice").toBool()) return;

  if (RentalisSettings::invoice_warning("water_price").toBool()
      && ui->dsb_water_price->value() <= RentalisSettings::invoice_warning("water_price_threshold").toFloat()) {
  ...
  if (RentalisSettings::invoice_warning("water_subscription").toBool()
      && ui->dsb_water_subscription->value()
             <= RentalisSettings::invoice_warning("water_subscription_threshold").toFloat()) {
  ...
  if (RentalisSettings::invoice_warning("water_last_check").toBool()
      && ui->sb_last_water_check->value()
             <= RentalisSettings::invoice_warning("water_last_check_threshold").toFloat()) {
  ...
  if (RentalisSettings::invoice_warning("water_check").toBool()
      && ui->sb_water_check->value() <= RentalisSettings::invoice_warning("water_check_threshold").toFloat()) {
  ...
  if (RentalisSettings::general_date && RentalisSettings::invoice_warning("dates").toBool()
